...
void merge(struct pokemon **pokemones, int pos_inicio, int pos_mitad, int pos_fin, bool *error_memoria)
{
	...
	struct pokemon **pokemones_aux = malloc((size_t)(tamanio) * sizeof(struct pokemon*));
...
void merge_sort(struct pokemon **pokemones, int pos_inicio, int pos_fin, bool *error_memoria)
{
        if (!pokemones || !error_memoria)
                return;
		...
		if(!(*error_memoria)) 
                        merge(pokemones, pos_inicio, pos_mitad, pos_fin, error_memoria);
	}
}
