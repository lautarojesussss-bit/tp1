...
void prueba_guardar_archivo()
{
	...
	tp1_t *tp_aux = NULL;
        ...
        const char *nombre_archivo = "pruebas/archivos_csv/pokemones_desordenados.csv";
        const char *archivo_escribir = "pruebas/archivos_csv/pokemones_escritos.csv";
...
void prueba_buscar_n()
{
        ...
        if (tp_aux != NULL) {
                pokemon_aux = tp1_buscar_orden(tp_aux, 1);
                cumple = es_el_mismo_pokemon(pokemon_aux, &pokemon_correcto_1);
                tp1_destruir(tp_aux);
        }

        pa2m_afirmar(cumple, "Encontrar un pokemon en un tp1 mediante su posición por orden alfábetico.");
}
